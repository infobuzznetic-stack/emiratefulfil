...
async function fetchCatalog() {
  ...
  return data.map((p) => ({ id: p.id, name: p.name, category: p.category, cost: Number(p.cost), sell: Number(p.sell), emoji: p.emoji, description: p.description, image_url: p.image_url, images: Array.isArray(p.images) ? p.images : [], stock: Number(p.stock ?? 0), assignedSellerEmail: p.assigned_seller_email || null }));
}
...
function Dashboard({ session, onLogout, notify, initialTab, onTabChange }) {
  ...
  // Products the Admin has assigned to one specific seller only show up for
  // that seller (and for Admin, who manages everything). Everyone else still
  // sees the regular, unrestricted catalog exactly as before.
  const visibleCatalog = isAdmin
    ? catalog
    : catalog.filter((p) => !p.assignedSellerEmail || p.assignedSellerEmail === session.email);
...
function AdminTab({ catalog, sellerCount, notify, onCatalogChanged }) {
  const [form, setForm] = useState({ name: "", category: "", cost: "", sell: "", emoji: "📦", description: "", images: [], stock: "", assignedSeller: "" });
  ...
  const addProduct = async (e) => {
    ...
    const { error } = await supabase.from("products").insert({
      ...
      assigned_seller_email: form.assignedSeller || null,
    });
    if (error) { notify("Could not add product."); return; }
    setForm({ name: "", category: "", cost: "", sell: "", emoji: "📦", description: "", images: [], stock: "", assignedSeller: "" });
    notify(form.assignedSeller ? `Product added — only visible to ${form.assignedSeller}.` : "Product added to catalog.");
    onCatalogChanged();
  };
  ...
  const startEdit = (p) => {
    setEditingId(p.id);
    const images = Array.isArray(p.images) && p.images.length ? p.images : (p.image_url ? [p.image_url] : []);
    setEditForm({ name: p.name || "", category: p.category || "", cost: p.cost, sell: p.sell, emoji: p.emoji || "📦", description: p.description || "", images, stock: p.stock ?? 0, assignedSeller: p.assignedSellerEmail || "" });
  };
  const cancelEdit = () => { setEditingId(null); setEditForm(null); };
  const uploadEditImages = async (e) => {
    const files = Array.from(e.target.files || []);
  ...
  const saveEdit = async (id) => {
    ...
    const { error } = await supabase.from("products").update({
      ...
      images, image_url: images[0] || null,
      stock: Math.max(0, parseInt(editForm.stock, 10) || 0),
      assigned_seller_email: editForm.assignedSeller || null,
    }).eq("id", id);
    ...
    <div>
      ...
      <form onSubmit={addProduct} className="mt-8 rounded-2xl p-6 bg-white grid sm:grid-cols-6 gap-3 items-end" style={{ border: "1px solid #E5E7EB" }}>
        ...
        <div className="sm:col-span-2">
          ...
          <select value={form.assignedSeller} onChange={(e) => setForm({ ...form, assignedSeller: e.target.value })} className="mt-1 w-full rounded-lg px-3 py-2 text-sm" style={{ border: "1px solid #E5E7EB" }}>
            <option value="">All sellers (public catalog)</option>
            {sellers.map((s) => (
              <option key={s.id} value={s.email}>{s.email}{s.store_name ? ` — ${s.store_name}` : ""}</option>
            ))}
          </select>
        </div>
            ...
            {editingId === p.id ? (
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <select value={editForm.assignedSeller} onChange={(e) => setEditForm({ ...editForm, assignedSeller: e.target.value })} className="mt-0.5 w-full rounded-lg px-2 py-1.5 text-xs" style={{ border: "1px solid #E5E7EB" }}>
                    <option value="">All sellers (public catalog)</option>
                    {sellers.map((s) => (
                      <option key={s.id} value={s.email}>{s.email}{s.store_name ? ` — ${s.store_name}` : ""}</option>
                    ))}
              ...
              <>
                ...
                <div className="mt-1 text-[11px] font-semibold truncate" style={{ color: p.assignedSellerEmail ? "#F8B400" : "#9CA3AF" }} title={p.assignedSellerEmail || ""}>
                  {p.assignedSellerEmail ? `🔒 Only ${p.assignedSellerEmail}` : "🌐 Visible to all sellers"}
                </div>
